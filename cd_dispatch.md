...
**4.** Before starting the script, please read the `config.lua` (located inside the main cd_dispatch folder), the `config.js` (located inside the html folder) and the `customise_me` (located inside the server and client folders) and configure the script to suit your servers needs.
